...
export class PembayaranComponent implements OnInit {
  ...
  getPaketUndangan() {
    ...
    this.dashboardSvc.list(DashboardServiceType.MNL_MD_PACK_INVITATION,).subscribe({
      next: (res) => {
        console.log('[PembayaranAdmin] Raw paket-undangan response:', res);
        this.paketList = res?.data ?? [];
        this.getDetailUser();
      },
      error: (error) => {
        console.error('[PembayaranAdmin] Error paket-undangan response:', {
          status: error?.status,
          url: error?.url,
          error: error?.error,
        });
  ...
  getDetailUser() {
    ...
    this.dashboardSvc.getParam(DashboardServiceType.ADM_IDX_DASHBOARD, '').subscribe({
      next: (res) => {
        console.log('[PembayaranAdmin] Raw get-users response:', res);
      ...
      error: (error) => {
        console.error('[PembayaranAdmin] Error get-users response:', {
          status: error?.status,
          url: error?.url,
          error: error?.error,
        });
