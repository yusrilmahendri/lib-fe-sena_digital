...
export class TampilanComponent implements OnInit, OnDestroy {
  ...
  private resolvePackagePrice(tier: string | null | undefined): number | null {
    ...
    const matchedPackage = this.resolvePackageCatalogEntry(normalizedTier);
    const price = Number(matchedPackage?.price ?? matchedPackage?.harga ?? matchedPackage?.amount);
    return Number.isFinite(price) ? price : null;
  }

  private resolvePackageCatalogEntry(tier: PaidPackageTier): any | null {
    return this.packageCatalog.find((paket) => resolvePackageTier(paket) === tier) || null;
  }
  ...
  private resolveTargetPackagePriceInfo(rawTargetPackage: unknown, tier: PaidPackageTier): {
    ...
    const rawPackageObject = rawTargetPackage && typeof rawTargetPackage === 'object' ? rawTargetPackage as any : null;
    const catalogPackage = this.resolvePackageCatalogEntry(tier);
    const packageObject = rawPackageObject?.upgrade_pricing || rawPackageObject?.price || rawPackageObject?.harga
      ? rawPackageObject
      : catalogPackage;
    const pricing = packageObject?.upgrade_pricing || {};
    ...
    return {
      normalPrice,
      originalPrice,
      originalPriceLabel: this.formatCurrencyLabel(originalPrice),
      discountPercentage,
      discountAmount,
      discountAmountLabel: this.formatCurrencyLabel(discountAmount),
      upgradePrice,
      upgradePriceLabel: this.formatCurrencyLabel(upgradePrice),
    };
  }
